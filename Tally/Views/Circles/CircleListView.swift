        ...
        VStack(alignment: .leading, spacing: TallySpacing.sm) {
            HStack(alignment: .center) {
                ...
                HStack(spacing: TallySpacing.sm) {
                    ...
                    GlassNavButton(icon: "plus", action: onAdd)
                }
            }
        ...
        LazyVStack(spacing: CardMetrics.cardSpacing) {
            ForEach(Array(filteredCircles.enumerated()), id: \.element.id) { index, circle in
                NavigationLink(value: circle) {
                    CircleListCard(circle: circle, index: index, favoriteIDs: $favoriteIDs)
                }
                .buttonStyle(.plain)
            }
        }
...
// MARK: - Circle List Card (DoorDash-style)

private struct CircleListCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // ── Accent strip + Image (clipped together) ──
            VStack(spacing: 0) {
                accentStrip
                imageContent
            }
            .clipShape(RoundedRectangle(cornerRadius: CardMetrics.cardCornerRadius))
    ...
    // MARK: - Content Area (Floating, Transparent)
                ...
                HStack(spacing: TallySpacing.md) {
                    Button { toggleFavorite() } label: {
                        Image(systemName: isFavorited ? "heart.fill" : "heart")
                            .font(TallyIcon.xl)
                            .foregroundStyle(isFavorited ? .red : TallyColors.textSecondary)
                    }
                    .buttonStyle(.plain)
